...
static int bar(const char *re, int re_len, const char *s, int s_len,
  ...
  for (i = j = 0; i < re_len && j <= s_len; i += step) {
	  ...
	  step = 1; //FIXME: this looks like a bug, we should analyse and report it.
#else
...
int slre_compile(const char *re, int re_len, int flags,
                 struct slre_regex_info *info) {
  int i, step, depth = 0;
...
int slre_match(const char *regexp, const char *s, int s_len,
  ...
  DBG(("========================> [%s] [%.*s]\n", regexp, s_len, s));
  {
    ...
    maybe_error = slre_compile(regexp, (int)strlen(regexp), flags, &info);
