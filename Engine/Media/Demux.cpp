...
MediaProcController::MediaPacketPtr MediaProcController::MediaDemux::obtainPacket(MediaEntries index, bool &cacheReadStarted) {
	...
	return MediaPacketPtr(packet, MediaPacketDeleter{&media});
}
...
void MediaProcController::MediaDemux::demultiplexStreams(long double videoTimeBase) {
	...
	do {
		...
		auto packet = media.acquireMediaPacket();
		if (packet) {
			read_result = av_read_frame(media.formatContext, packet.get());
		} else {
			read_result = AVERROR(ENOMEM);
			sendToLog(LogLevel::Error, "Unable to allocate a demux packet\n");
		}

		SDL_AtomicUnlock(&async.loadPacketArraysQueue.resultsLock);

		MediaEntries entry = InvalidEntry;
		if (read_result >= 0 && packet && !(packet->flags & AV_PKT_FLAG_CORRUPT)) {
			media.getVideoTimecodes(counter, packet.get(), videoTimeBase);
		...
		pushPacket(entry, std::move(packet), read_result, demultiplexingComplete, videoTimeBase);

	} while (true);
}

void MediaProcController::MediaDemux::pushPacket(MediaEntries id, MediaPacketPtr packet, int read_result, bool &demultiplexingComplete, long double videoTimeBase) {
	...
	if (id == InvalidEntry) {
		// The custom deleter returns ignored packets to the pool.
	} else if (id != SubsEntry) {
		Lock lock(&packetQueue[id]);
		packetQueue[id].push_back(packet.release());
		queuedPacket = true;
